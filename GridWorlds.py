...
class GridWorld():
    ...
    def step(self, action):
        ...
        if not self.game_over(self.x, self.y, self.has_key, self.board):
            self.x, self.y, self.has_key, self.board, self.score, self.reward = self.move(self.x, self.y, self.has_key,
        ...
        for i in range(10):
            for j in range(10):
                ...
                pygame.draw.rect(self.screen, col,
                                 pygame.Rect(100 + 60 * i + border, 60 * j + border, 60 - 2 * border, 60 - 2 * border))
                if val > 0:
                    x = 105 + 60 * i
                    y = 5 + 60 * j
                    if val & 4:
                    ...
                    if val & 1:
                        if self.game_over(self.x, self.y, self.has_key, self.board) and not self.won(self.x, self.y,
                                                                                                     self.has_key,
                                                                                                     self.board):
    ...
    #Takes an action_list = [move,n] -> move = left,right,up,down, n = times to repeat it
    def move(self, x, y, has_key, board, score, direction=['left', 1]):
        xOriginal,yOriginal = x,y
        newx, newy = x, y
        for n in range(direction[1]):
            if direction[0] == 'left':
                 if x > 0:
                    newx = x - 1
                    x = x - 1
    ...
    def new_game(self):
        if self.seed>6: #6 is the number of seeds
            raise Exception("Seed does not exist")
        board = np.loadtxt(f'board{self.seed}.txt', dtype=int).T
        if board.shape != (10, 10) or np.sum(board == 2) != 1 or np.sum(board == 4) != 1:
            raise Exception(f'board{self.seed}.txt corrupt')
