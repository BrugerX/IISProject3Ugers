...
class DQNAgentXY:
    ...
    def __init__(self,
        ...
        self.true_Score_Averages = [] #For storing the average after each evaluation, idx * eval_Sample_Sizes = timestep at which we evaluated.
        self.eval_times = 0
    ...
    *Each layer in the connected linear layer is seperated by the activation function
    ...
    def initializeNeuralNetworks(self):
        ...
        self.tNet = NeuralNetwork(layers_shape=self.layers_shape, output_depth=self.actions_num,
                                  input_depth=input_depth, activation_function=self.activation_function,
    ...
    def follow_Optimal(self):
        ...
        accumulated_score = 0
        for x in range(self.eval_Sample_Size):
            ...
            score = stepTuple[1] * self.gamma ** self.k  # Calculate the score - discount
        ...
        average = accumulated_score / self.T_evaluate
        self.true_Score_Averages = self.true_Score_Averages + [average]
        end_time = time.perf_counter()-start_time
        self.eval_times = self.eval_times + end_time
...
class NeuralNetwork(torch.nn.Module):
    ...
    def makeModel(self):
        ...
        hiddenLayers = [self.activation_function, torch.nn.Linear(self.depth,
                                                                  self.depth)] * self.width  # There is no non-linear function between the last hidden layer and the output
...
class DQNAgentFullSpace:
    ...
    *Each layer in the connected linear layer is seperated by the activation function
    ...
    def Training_step(self):
        ...
        # 2. Choose between exploitation or exploration (Greedy or exploratory action)
        self.S_t = torch.tensor((self.env.board + (self.env.has_key)),dtype=torch.float32).flatten()
        a = self.greedVsExp()
        ...
        S_t1 = torch.tensor((self.env.board + stepTuple[0][2]),dtype=torch.float32).flatten()
    ...
    def follow_Optimal(self):
        ...
        accumulated_score = 0
        for x in range(self.eval_Sample_Size):
            state = torch.tensor((self.env.board + [self.env.has_key]),dtype=torch.float32).flatten()  # Get the state
            max_a = torch.argmax(self.pNet.forward(state)).item()  # Get the greedy policy
            stepTuple = self.env.step(self.actions_available[max_a])  # Step in the environment
            score = stepTuple[1] * self.gamma ** self.k  # Calculate the score - discount
            self.k += 1  # Increment the current non-terminal counter
