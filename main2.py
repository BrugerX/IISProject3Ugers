...
experiment_Names_list = ["XY_2","XY_4","XY_6","FS_2","FS_4","FS_6"]
env = GridWorld(seed=1)

for idx,exp_Nr in enumerate(experiment_Names_list): #Experiment main loop
    n = 100
    a = ((idx%3+1)*2) #2,4,6
    csv_Name = f"Pilot_Study_Feature_Vector{exp_Nr}"
    csv_Name_Time = f"Pilot_Study_Feature_Vector_Time{exp_Nr}"
    createCSV(csv_Name)
    ...
    epsilon_decay = (0.9)/4000
    save_Cutoff = 4
    if idx<3:
        dqnXY = DQNAgent(state_representation=state1, actions_List=actions, layers_shape=(520,1), env=env, load_Name=(names1), save_Name=(names1),T_train=2,
                         epsilon_start=1,save_Cutoff=save_Cutoff,epsilon_decay=epsilon_decay, eval_Sample_Sizes=n, T_evaluate=n,feature_type="XY",csv_Name=f"{csv_Name}")
    else:
        dqnXY = DQNAgent(state_representation=state2, actions_List=actions, layers_shape=(520,1), env=env, load_Name=(names1), save_Name=(names1),
                        epsilon_start=1,save_Cutoff=save_Cutoff,epsilon_decay=epsilon_decay, eval_Sample_Sizes=n, T_evaluate=n,csv_Name=f"{csv_Name}")
    ...
    time_dict = {"time_Step":time_train}
    timeDf = pd.DataFrame.from_dict(time_dict)
    timeDf.to_csv(f"{csv_Name_Time}",mode="a")
